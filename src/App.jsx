...
const formatNumber = (num) => {
  ...
  return Math.ceil(num).toLocaleString('it-IT');
};
...
const generateWeeklyReport = (kpi, periods) => {
  ...
  const addSection = (title, htmlContent, plainContent, status = 'neutral') => {
    ...
    reportHTML.push(
      `<div class="p-4 rounded-xl border-l-4 mb-3 ${colors[status]} border-${status === 'neutral' ? 'slate-400' : status === 'good' ? 'emerald-500' : status === 'warning' ? 'amber-500' : 'rose-500'}">
        <h4 class="font-bold text-sm uppercase mb-1 opacity-80">${title}</h4>
  ...
  const totalVolume = c.chat + c.ast_new + c.form;
  const prevVolume = p.chat + p.ast_new + p.form;
  const volDiff = totalVolume - prevVolume;
  const volTrend = volDiff >= 0 ? "in crescita" : "in calo";
  
  addSection("Panoramica Esecutiva", `La settimana <strong>${periods.curr.label}</strong> si chiude con un volume complessivo di attività <strong>${volTrend}</strong> rispetto alla precedente (<strong>${formatNumber(totalVolume)}</strong> vs ${formatNumber(prevVolume)} interazioni totali). Il carico di lavoro è stato distribuito prevalentemente sul reparto <strong>${c.chat > c.ast_new ? 'Chat' : 'Assistenza'}</strong>.`, `La settimana ${periods.curr.label} si chiude con un volume complessivo di attività ${volTrend} (Totale interazioni: ${totalVolume}). Il carico maggiore ha riguardato il reparto ${c.chat > c.ast_new ? 'Chat' : 'Assistenza'}.`, 'neutral');

  const chatDiff = c.chat - p.chat;
  const chatTrend = chatDiff >= 0 ? "+" : "";
  let chatAnalysis = `Il team ha gestito <strong>${formatNumber(c.chat)} conversazioni</strong>. `;
  if (c.chat > 0) {
    if (chatDiff > 0) { chatAnalysis += `Si registra un <strong>aumento della domanda del ${p.chat > 0 ? ((chatDiff/p.chat)*100).toFixed(0) : 100}%</strong>. Nonostante l'aumento dei volumi, il tempo medio di risposta si attesta a <strong>${formatTime(c.avg_resp)}</strong>, indicando una ${c.avg_resp < 2 ? 'eccellente reattività' : 'buona tenuta'} del team.`; } 
    else { chatAnalysis += `Il volume è in calo (${chatTrend}${formatNumber(chatDiff)}). Questo ha permesso al team di dedicare più tempo alla qualità, con una durata media di conversazione pari a <strong>${formatTime(c.avg_dur)}</strong>.`; }
    addSection("Performance Chat & Supporto Live", chatAnalysis, chatAnalysis.replace(/<[^>]*>/g, ''), 'good');
  }

  const astSaldo = c.ast_closed - c.ast_new;
  let astStatus = 'neutral';
  let astText = `Sono stati aperti <strong>${formatNumber(c.ast_new)} nuovi ticket</strong> e ne sono stati chiusi <strong>${formatNumber(c.ast_closed)}</strong>. `;
  if (c.ast_new > 0) {
    if (astSaldo >= 0) { astText += `Il saldo settimanale è <strong>positivo (+${astSaldo})</strong>, il che significa che stiamo smaltendo il pregresso. Il tempo di prima risposta è di <strong>${formatTime(c.ast_resp)}</strong>.`; astStatus = 'good'; } 
    else { astText += `⚠️ <strong>Attenzione al Backlog:</strong> sono entrati più ticket di quelli gestiti (Saldo: ${astSaldo}). È necessario accelerare le chiusure o analizzare i picchi di richieste.`; astStatus = 'bad'; }
    addSection("Ticket Assistenza", astText, astText.replace(/<[^>]*>/g, ''), astStatus);
  }

  if (c.dev_backlog > 0) {
    const devDiff = c.dev_backlog - p.dev_backlog;
    let devText = `Il backlog di sviluppo conta attualmente <strong>${formatNumber(c.dev_backlog)} ticket aperti</strong>. `;
    let devStatus = 'info';
    if (devDiff < 0) { devText += `Ottimo trend: il backlog è stato ridotto di <strong>${Math.abs(devDiff)}</strong> unità rispetto alla settimana scorsa.`; devStatus = 'good'; } 
    else if (devDiff > 0) { devText += `Il debito tecnico è aumentato di ${devDiff} unità. Il tempo medio di risoluzione attuale è <strong>${formatTime(c.dev_res)}</strong>.`; devStatus = 'warning'; }
    addSection("Sviluppo & Bug Fixing", devText, devText.replace(/<[^>]*>/g, ''), devStatus);
  }

  if (c.form > 0) {
    let formText = `Il reparto Customer Success ha erogato <strong>${formatNumber(c.form)} sessioni</strong> per un totale di <strong>${formatTime(c.form_hours*60)}</strong> di formazione. `;
    let formStatus = 'good';
    if (c.noshow > 0) { formText += `❌ Da segnalare <strong>${formatNumber(c.noshow)} casi di No-Show</strong> che hanno impattato sull'efficienza delle agende.`; formStatus = 'warning'; } 
    else { formText += `👏 Nessun No-Show registrato: ottima gestione delle conferme appuntamenti.`; }
    addSection("Onboarding & Formazione", formText, formText.replace(/<[^>]*>/g, ''), formStatus);
  }
    ...
    <div className="flex-1 w-full relative min-h-0">
      {isEmpty ? (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-300">
          ...
          <span className="text-xs font-medium">Dati insufficienti</span>
        </div>
      ) : (
...
const ComparisonRow = ({ label, current, previous, unit = '', invert = false, isTeamMetric = false, isTime = false }) => {
  const valCurr = isTime ? current : Math.ceil(current);
  const valPrev = isTime ? previous : Math.ceil(previous);
  const diff = valCurr - valPrev;
  const perc = valPrev !== 0 ? ((diff / valPrev) * 100) : 0;
  const isPositive = invert ? diff <= 0 : diff >= 0;
  const color = isPositive ? 'text-emerald-600 bg-emerald-50' : 'text-rose-600 bg-rose-50';
  const displayValue = isTime ? formatTime(valCurr) : formatNumber(valCurr);
  const displayDiff = isTime ? formatTime(Math.abs(diff)) : Math.abs(diff).toFixed(0);
  ...
  return (
    <div className={`flex items-center justify-between p-4 bg-white border border-slate-100 rounded-xl shadow-sm ${isTeamMetric ? 'opacity-90 bg-slate-50/50' : ''}`}>
      <div className="flex flex-col">
        <span className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-wider truncate">
          {label} {isTeamMetric && <span className="bg-slate-200 text-slate-600 text-[9px] px-1.5 py-0.5 rounded">TEAM</span>}
        </span>
        <span className="text-2xl font-bold text-slate-800 mt-1">
          {displayValue} <span className="text-sm font-normal text-slate-400">{unit}</span>
        </span>
      </div>
...
const LeaderboardRow = ({ rank, name, chats, formations, score }) => (
  <div className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-lg transition-colors border-b border-slate-50 last:border-0">
    <div className="flex items-center gap-3">
      <div className={`w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold ${rank === 1 ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>
        {rank}
      </div>
      <div className="flex flex-col">
        <span className="text-sm font-bold text-slate-700">{name}</span>
        <span className="text-[10px] text-slate-400">Score: {formatNumber(score)}</span>
      </div>
    </div>
    <div className="flex gap-4 text-xs font-medium">
      <div className="text-right w-16">
        <span className="block text-slate-800">{formatNumber(chats)}</span>
        <span className="block text-[9px] text-slate-400">CHAT</span>
      </div>
      <div className="text-right w-16">
        <span className="block text-slate-800">{formatNumber(formations)}</span>
        <span className="block text-[9px] text-slate-400">FORM</span>
      </div>
    </div>
  </div>
);

const DebugConsole = ({ logs, clearLogs, show }) => {
  const scrollRef = useRef(null);
  useEffect(() => { if (scrollRef.current) scrollRef.current.scrollTop = scrollRef.current.scrollHeight; }, [logs]);
  if (!show) return null;
  return (
    <div className="fixed bottom-0 left-0 right-0 h-40 bg-slate-900 text-slate-300 font-mono text-[10px] z-50 shadow-2xl border-t border-slate-700 flex flex-col opacity-95">
      <div className="flex justify-between items-center px-4 py-1 bg-slate-800 border-b border-slate-700">
        <span className="font-bold flex items-center gap-2 text-white"><Terminal size={12}/> Console Debug</span>
        <button onClick={clearLogs} className="text-xs hover:text-white">Pulisci</button>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-2 space-y-1">
        {logs.map((log, i) => <div key={i}>[{format(log.time,'HH:mm:ss')}] {log.msg}</div>)}
      </div>
    </div>
  );
};
...
export default function App() {
  ...
  const periods = useMemo(() => {
    ...
    const endPrev = subWeeks(endCurr, 1);
    return { 
      curr: { start: startCurr, end: endCurr, label: `Sett. ${getISOWeek(currentDate)} (${format(startCurr, 'dd MMM')} - ${format(endCurr, 'dd MMM')})` }, 
  ...
  const kpi = useMemo(() => {
    const calc = (start, end) => {
      ...
      return {
        ...
        avg_resp: avgResp,
        avg_dur: avgDur,
        form: inRangeForm.length,
        form_hours: inRangeForm.reduce((a,b)=>a+Number(b.duration||0),0)/60,
  ...
  const dailyMacroTrend = useMemo(() => {
    const s = startOfDay(periods.curr.start);
    const e = endOfDay(periods.curr.end);
    ...
    return days.map(day => {
      ...
      if (devItems.length > 0) lastDev = Number(devItems[devItems.length - 1].backlog);

      return { date: format(day, 'EEE', {locale: it}), chat: Math.ceil(chatVol), form: formVol, ast: Math.ceil(astVol), dev: lastDev || null };
    });
  }, [data, periods, selectedOperator]);

  const operatorFormations = useMemo(() => {
    if (selectedOperator === 'all') return [];
    return data.form.filter(f => f.operator === selectedOperator && safeInRange(f.date, periods.curr.start, periods.curr.end))
      .sort((a,b) => new Date(b.date) - new Date(a.date));
  }, [data, selectedOperator, periods]);

  const operatorStats = useMemo(() => {
    const stats = {};
    data.chat.filter(x => safeInRange(x.import_date, periods.curr.start, periods.curr.end))
      .forEach(x => { stats[x.operator] = (stats[x.operator] || 0) + Number(x.chats_accepted); });
    return Object.entries(stats).map(([name, val]) => ({ name, val })).sort((a,b) => b.val - a.val).slice(0,10);
  }, [data, periods]);

  const leaderboard = useMemo(() => {
    const stats = {};
    data.chat.filter(x => safeInRange(x.import_date, periods.curr.start, periods.curr.end)).forEach(x => {
      if(!stats[x.operator]) stats[x.operator] = { chat: 0, form: 0 };
      stats[x.operator].chat += Number(x.chats_accepted);
    });
    data.form.filter(x => safeInRange(x.date, periods.curr.start, periods.curr.end)).forEach(x => {
      if(!stats[x.operator]) stats[x.operator] = { chat: 0, form: 0 };
  ...
  const handleFile = async (e, type) => {
    ...
    if (!file) return;
    try {
      const res = await parseExcel(file, type);
      setUploadModal({ type, rows: res.rows, file: file.name, range: res.range });
  ...
  const confirmUpload = async () => {
    ...
    setLoading(true);
    try {
      const { type, rows } = uploadModal;
      let table = '', conflict = '', dateCol = '', finalRows = rows, start, end;
      ...
      if (type === 'chat') {
        ...
        if (sVal && eVal) { start = new Date(sVal); end = new Date(eVal); }
        else if (uploadModal.range) { start = uploadModal.range.start; end = uploadModal.range.end; }
        else throw new Error("Range mancante");
        
        const days = eachDayOfInterval({ start, end });
        finalRows = [];
        rows.forEach(r => {
          const dailyChat = Number(r.chats_accepted) / days.length;
          days.forEach(d => finalRows.push({ operator: r.operator, chats_accepted: dailyChat, hours_worked: r.hours_worked, avg_response_time: r.avg_response_time, avg_duration: r.avg_duration, import_date: format(d, 'yyyy-MM-dd') }));
        });
        setCurrentDate(start);
      }
      else if (type === 'formazioni') {
        table = 'formazioni'; conflict = 'id'; dateCol = 'date';
        if (rows.length === 0) throw new Error("File vuoto");
        ...
        await supabase.from('manual_noshows').upsert({ week_id: weekId, count: nsCount });
        setCurrentDate(start);
      }
      else {
        table = type === 'assistenza' ? 'ticket_assistenza' : 'ticket_sviluppo'; 
        dateCol = 'date'; conflict = 'date';
        const dates = rows.map(r => parseISO(r.date));
        start = min(dates); end = max(dates);
      }
      ...
      const CHUNK = 500;
      for (let i = 0; i < finalRows.length; i += CHUNK) await supabase.from(table).upsert(finalRows.slice(i, i + CHUNK), { onConflict: conflict });
      await fetchAll();
      setUploadModal(null);
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      <div className="w-64 bg-white border-r border-slate-200 flex flex-col z-20 shadow-xl">
        <div className="p-6 flex-1">
          <div className="flex items-center gap-3 mb-8">
            <div className="bg-indigo-600 p-2 rounded-lg"><Database className="text-white" size={20} /></div>
            <h1 className="font-bold text-lg tracking-tight">Pienissimo<span className="text-indigo-600">.bi</span></h1>
          </div>
          <div className="space-y-1">
            ...
            <SidebarItem icon={AlertCircle} label="Assistenza" active={view === 'assistenza'} onClick={() => setView('assistenza')} />
            <SidebarItem icon={Code} label="Sviluppo" active={view === 'sviluppo'} onClick={() => setView('sviluppo')} />
          </div>
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-md border-b border-slate-200 px-8 py-4 flex justify-between items-center z-10 sticky top-0">
          <div className="flex items-center gap-4">
             ...
             <div className="flex items-center gap-2">
                ...
                <select className="bg-white border border-slate-200 text-sm rounded-lg p-1.5 outline-none font-medium text-slate-700 cursor-pointer"
          ...
          {loading && <div className="flex items-center gap-2 text-indigo-600 text-xs font-bold animate-pulse"><Loader2 size={14} className="animate-spin"/> Syncing...</div>}
        </div>
          ...
          <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {view === 'dashboard' && (
              <>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-2">
                  <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white rounded-2xl p-6 border border-indigo-100 shadow-sm relative overflow-hidden">
                      <div className="absolute top-0 right-0 p-10 opacity-[0.03] text-indigo-600"><Sparkles size={120}/></div>
                      <div className="flex justify-between items-center mb-6 relative z-10">
                        <div className="flex items-center gap-3">
                          <div className="bg-indigo-50 p-2.5 rounded-xl text-indigo-600"><Mail size={20}/></div>
                          <div><h3 className="font-bold text-slate-800">Bollettino Manageriale</h3><p className="text-xs text-slate-500">Analisi AI Narrativa</p></div>
                        </div>
                        <div className="flex gap-2">
                          {!generatedReport && <button onClick={handleGenerateReport} disabled={isGenerating} className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg text-xs font-bold hover:bg-slate-800 transition-all shadow-md active:scale-95">{isGenerating ? <Loader2 className="animate-spin" size={14}/> : <Sparkles size={14} />} Genera Analisi</button>}
                          {generatedReport && <button onClick={copyToClipboard} className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-lg text-xs font-bold hover:bg-emerald-100 transition-all"><Copy size={14} /> Copia Testo Email</button>}
                        </div>
                      </div>

                      {generatedReport ? (
                        <div className="space-y-4 relative z-10 animate-in fade-in slide-in-from-bottom-2 text-slate-700">
                        ...
                        <div className="h-32 flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-100 rounded-xl">
                          <Info size={24} className="mb-2 opacity-50"/><p className="text-xs">Clicca "Genera Analisi" per creare il report discorsivo</p>
                        </div>
                      )}
                    </div>
                    ...
                    {selectedOperator !== 'all' && (
                      <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
                        <h4 className="font-bold text-slate-700 mb-4 flex items-center gap-2"><ClipboardList size={16}/> Registro Formazioni ({selectedOperator})</h4>
                        <div className="overflow-hidden rounded-lg border border-slate-100">
                          <table className="w-full text-sm text-left">
                            <thead className="bg-slate-50 text-slate-500 font-medium"><tr><th className="px-4 py-2">Data</th><th className="px-4 py-2">Argomento</th><th className="px-4 py-2 text-right">Minuti</th></tr></thead>
                            <tbody className="divide-y divide-slate-100">
                              {operatorFormations.length > 0 ? operatorFormations.map((f, i) => (
                                <tr key={i} className="hover:bg-slate-50/50">
                                  <td className="px-4 py-3 font-medium text-slate-700">{format(new Date(f.date), 'dd MMM')}</td>
                                  <td className="px-4 py-3 text-slate-600 truncate max-w-xs" title={f.topic}>{f.topic}</td>
                                  <td className="px-4 py-3 text-right font-mono text-xs bg-slate-50 text-indigo-600 font-bold">{f.duration}</td>
                                </tr>
                              )) : <tr><td colSpan="3" className="px-4 py-8 text-center text-slate-400 italic">Nessuna formazione registrata.</td></tr>}
                            </tbody>
                          </table>
                        </div>
                      </div>
                  ...
                  <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm h-full flex flex-col" style={{minHeight: '400px'}}>
                    ...
                    {selectedOperator === 'all' ? (
                      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                        {leaderboard.length > 0 ? leaderboard.map((op, idx) => <LeaderboardRow key={op.name} rank={idx + 1} name={op.name} chats={op.chat} formations={op.form} score={op.score} />) : <div className="text-center text-slate-400 mt-10 text-xs">Nessun dato in questa settimana.</div>}
                      </div>
                    ) : (
                      <div className="text-center p-4">
                        <div className="w-20 h-20 mx-auto rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center text-3xl font-bold mb-4">{selectedOperator.charAt(0)}</div>
                        <h3 className="text-xl font-bold text-slate-900">{selectedOperator}</h3>
                        <p className="text-xs text-slate-500 uppercase tracking-widest mb-6">Support Specialist</p>
                        <div className="w-full bg-slate-50 p-4 rounded-xl mb-2 border border-slate-100"><p className="text-xs text-slate-400 font-bold uppercase">Volume Chat</p><p className="text-2xl font-bold text-indigo-600">{formatNumber(kpi.curr.chat)}</p></div>
                        <div className="w-full bg-slate-50 p-4 rounded-xl border border-slate-100"><p className="text-xs text-slate-400 font-bold uppercase">Ore Formazione</p><p className="text-2xl font-bold text-purple-600">{formatTime(kpi.curr.form_hours * 60)}</p></div>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="mt-6">
                  <ChartContainer title={`Andamento Operativo Giornaliero (${selectedOperator === 'all' ? 'Vista Globale' : 'Focus Operatore'})`} isEmpty={dailyMacroTrend.every(x => x.chat === 0 && x.ast === 0 && x.form === 0)}>
                    <ComposedChart data={dailyMacroTrend}>
                      ...
                      {selectedOperator === 'all' && <YAxis yAxisId="right" orientation="right" axisLine={false} tickLine={false} tick={{fill:'#64748b', fontSize:12}} />}
                      <Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{borderRadius:'8px', border:'none', boxShadow:'0 4px 12px rgba(0,0,0,0.1)'}} />
                      <Legend wrapperStyle={{paddingTop:'10px'}} />
                      
                      <Bar yAxisId="left" dataKey="chat" name="Chat Gestite" fill="#6366f1" radius={[4,4,0,0]} barSize={20} />
                      <Bar yAxisId="left" dataKey="form" name="Formazioni" fill="#8b5cf6" radius={[4,4,0,0]} barSize={20} />
                      {selectedOperator === 'all' && <Bar yAxisId="left" dataKey="ast" name="Ticket Chiusi" fill="#10b981" radius={[4,4,0,0]} barSize={20} />}
                      {selectedOperator === 'all' && <Line yAxisId="right" connectNulls type="monotone" dataKey="dev" name="Backlog Sviluppo" stroke="#f59e0b" strokeWidth={3} dot={{r: 4}} activeDot={{r: 6}} />}
                    </ComposedChart>
                  </ChartContainer>
                </div>
              </>
      ...
      <DebugConsole logs={logs} clearLogs={() => setLogs([])} show={showDebug} />
